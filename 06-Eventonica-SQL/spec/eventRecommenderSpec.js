describe("EventRecommender", () => {
  const { EventRecommender } = require("../public/eventRecommender"); // Update with your class names and file name
  let er;
  ...
  describe("saveUserEvent", () => {
    it("adds an event to a user's personal event array", () => {
      ...
      er.saveUserEvent(user.id, event.id); // change these to match your method signature
      expect(user.personalEvents.length).toEqual(1);
      expect(user.personalEvents[0].id).toEqual(event.id);
    });
  });
