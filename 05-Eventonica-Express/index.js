...
const { datesAreSameDay } = require("./utils/functions");
...
app
  ...
  .get((req, res) => {
    ...
    if (date) {
      events = events.filter(event => {
        console.log(`event date: ${event.date}, date: ${date}`);
        return datesAreSameDay(event.date, date);
      });
    }
  ...
  .post((req, res) => {
    ...
    if (!name) res.status(400).send("ERROR: Events need a name");
    else {
